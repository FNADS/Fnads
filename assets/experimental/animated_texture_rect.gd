		...
		if sprite_frames != null:
			if sprite_frames.get_frame_count(animation) >= 1:
				if frame > sprite_frames.get_frame_count(animation)-1:
					printerr("Node( ", self, " ), cannot set frame! 'frame' not found in 'animation'! ");
					...
					texture = sprite_frames.get_frame_texture(animation, frame);
					frame = value;
			else: printerr("Node( ", self, " ), cannot set frame! 'sprite_frames' doesn't contain any frames! ");
		else: printerr("Node( ", self, " ), cannot set frame! 'sprite_frames' is null! ");
